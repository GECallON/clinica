...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($usuarioModel->update($id, $_POST)) {
...
<head>
    ...
    <title>Editar Usuário - MedAgenda Pro</title>
                ...
                <section class="flex items-center justify-between gap-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="page-subtitle mt-1">Atualize dados de acesso e contato.</p>
                        </div>
                    </div>
                </section>
                    ...
                    <form method="POST" class="space-y-6" x-data="{ showPassword: false }">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="form-field md:col-span-2">
                                ...
                                <input id="nome" name="nome" type="text" class="w-full" value="<?= htmlspecialchars($usuario['nome']) ?>" required>
                            </div>

                            <div class="form-field">
                                <label for="email"><i class="fas fa-envelope mr-2 text-indigo-500 text-xs"></i>Email *</label>
                                <input id="email" name="email" type="email" class="w-full" value="<?= htmlspecialchars($usuario['email']) ?>" required>
                            </div>

                            <div class="form-field">
                                <label for="telefone"><i class="fas fa-phone mr-2 text-indigo-500 text-xs"></i>Telefone</label>
                                <input id="telefone" name="telefone" type="text" class="w-full" value="<?= htmlspecialchars($usuario['telefone']) ?>">
                            </div>

                            <div class="form-field relative">
                                <label for="senha"><i class="fas fa-lock mr-2 text-indigo-500 text-xs"></i>Nova senha <span class="text-[11px] text-slate-400">(opcional)</span></label>
                                <input :type="showPassword ? 'text' : 'password'" id="senha" name="senha" class="w-full pr-10" placeholder="Nova senha...">
                            ...
                            <div class="form-field">
                                ...
                                <select id="nivel_acesso" name="nivel_acesso" class="w-full" required>
                                    <option value="medico" <?= $usuario['nivel_acesso'] === 'medico' ? 'selected' : '' ?>>Médico</option>
                            ...
                            <div class="md:col-span-2">
                                <label class="flex items-center gap-2 text-sm font-medium text-slate-600">
                                    <input type="checkbox" name="ativo" value="1" class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-400" <?= $usuario['ativo'] ? 'checked' : '' ?>>
                                    Usuário ativo
                                </label>
                            </div>
                        </div>
